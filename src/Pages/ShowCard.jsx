...
const ShowCard = ({ formData }) => {
  const cardRef = useRef(null);
  console.log("formData", formData);
  const navigate = useNavigate();
  const [showQR, setShowQR] = useState(false);
  const cardURL = `${window.location.origin}/card/${formData.username}`;
  const [showPaymentPopup, setShowPaymentPopup] = useState(false);

  const handleDownload = async () => {
    if (!cardRef.current) return;

    const loadImages = cardRef.current.querySelectorAll("img");
    await Promise.all(
      Array.from(loadImages).map(
        (img) =>
          new Promise((resolve) => {
            if (img.complete) return resolve();
            img.onload = () => resolve();
            img.onerror = () => resolve(); // Still resolve to avoid breaking
          })
      )
    );
    ...
    link.href = dataUrl;
    link.download = `${formData.fullName || "card"}.png`;
    link.click();
  };
  ...
  const parseToArray = (input) => {
    if (Array.isArray(input)) {
      return input.flatMap((i) => i.split(",")).map((s) => s.trim());
    }
  ...
  // Safe fallback for backgroundColor
  ...
  return (
    <div className="showcard">
      ...
      <div className="buttons">
        ...
        <button className="button" onClick={handleGoLive}>
          Go Live
        </button>
        <button className="button" onClick={() => setShowQR(true)}>
          QR Code
        </button>
      </div>
      {showQR && (
        <div className="qr-modal">
          <div className="qr-content">
            <h3>Scan to View Your Card</h3>
            <QRCode value={cardURL} size={256} />
            <p>{cardURL}</p>
            <button onClick={() => setShowQR(false)} className="button">
              Close
            </button>
          </div>
        </div>
